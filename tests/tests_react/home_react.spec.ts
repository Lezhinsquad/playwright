...
test('검색_버튼UI 노출확인_kr', async ({ page }) => {
  ...
  const button = await page.$('.style_supportsItem__OIhu2.style_supportsItem__search__ZPNGK'); //검색창 노출 버튼 요소
  const buttonText = await button?.textContent(); // search_id에 저장된 텍스트를 value에 저장 
  expect(buttonText).toBe("검색창 열기"); //value 에 저장된 텍스트와 실제 노출되어야할 텍스트 비교
...
test('검색_버튼UI 노출확인_jp', async ({ page }) => {
  ...
  const button = await page.$('.style_supportsItem__OIhu2.style_supportsItem__search__ZPNGK'); //검색창 노출 버튼 요소
  const buttonText = await button?.textContent(); // search_id에 저장된 텍스트를 value에 저장 
  expect(buttonText).toBe("検索窓を開く"); //value 에 저장된 텍스트와 실제 노출되어야할 텍스트 비교
...
test('검색_버튼UI 노출확인_us', async ({ page }) => {
  ...
  const button = await page.$('.style_supportsItem__OIhu2.style_supportsItem__search__ZPNGK'); //검색창 노출 버튼 요소
  const buttonText = await button?.textContent(); // search_id에 저장된 텍스트를 value에 저장 
  expect(buttonText).toBe("Open Search Window"); //value 에 저장된 텍스트와 실제 노출되어야할 텍스트 비교
...
test('검색_Placeholder 확인_kr', async ({ page }) => {
  ...
  //Placeholder 검증
  const placeholderText = await page.getAttribute('.style_gnbSearch__input__KGtkv', 'placeholder'); // 검색창 활성화 상태에서 placeholder 값 얻기
  expect(placeholderText).toBe("작품, 작가, 출판사, 태그 검색"); // placeholder 값 비교
...
test('검색_Placeholder 확인_ja', async ({ page }) => {
  ...
  //Placeholder 검증
  const placeholderText = await page.getAttribute('.style_gnbSearch__input__KGtkv', 'placeholder'); // 검색창 활성화 상태에서 placeholder 값 얻기
  expect(placeholderText).toBe("作品, 作家, 出版社, タグ"); // placeholder 값 비교
...
test('검색_Placeholder 확인_us', async ({ page }) => {
  ...
  //Placeholder 검증
  const placeholderText = await page.getAttribute('.style_gnbSearch__input__KGtkv', 'placeholder'); // 검색창 활성화 상태에서 placeholder 값 얻기
  expect(placeholderText).toBe("Type to begin searching"); // placeholder 값 비교
...
test('검색_레이어 호출후 닫기_kr', async ({ page }) => {
  ...
  const button = await page.$('.style_supportsItem__OIhu2.style_supportsItem__search__ZPNGK'); //검색창 노출 버튼 요소
  const buttonText = await button?.textContent(); // search_id에 저장된 텍스트를 value에 저장 
  expect(buttonText).toBe("검색창 열기"); //buttonText 에 저장된 텍스트를 확인하여 닫기 버튼 텍스트가 미노출되는것으로 비교
...
test('검색_레이어 호출후 닫기_ja', async ({ page }) => {
  ...
  const button = await page.$('.style_supportsItem__OIhu2.style_supportsItem__search__ZPNGK'); //검색창 노출 버튼 요소
  const buttonText = await button?.textContent(); // search_id에 저장된 텍스트를 value에 저장 
  expect(buttonText).toBe("検索窓を開く"); //buttonText 에 저장된 텍스트를 확인하여 닫기 버튼 텍스트가 미노출되는것으로 비교
...
test('검색_레이어 호출후 닫기_us', async ({ page }) => {
 ...
 const button = await page.$('.style_supportsItem__OIhu2.style_supportsItem__search__ZPNGK'); //검색창 노출 버튼 요소
 const buttonText = await button?.textContent(); // search_id에 저장된 텍스트를 value에 저장 
 expect(buttonText).toBe("Open Search Window"); //buttonText 에 저장된 텍스트를 확인하여 닫기 버튼 텍스트가 미노출되는것으로 비교
...
test('랭킹 영역 노출 확인_KR', async ({ page }) => {
  ...
  expect(elementHandle).toBeTruthy; //elementHandle 실제 노출되는 페이지 노출되는지 비교 체크
...
//취향설정하기 버튼 노출
  const spanText = await page.$eval('span.style_myGenreSet__btnSubText__hgWM1', (element) => element.textContent); //취향 설정하기 버튼의 텍스트 요소 를 spanText에 저장
...
//취향 설정 하기 텍스트 변수 선언
  const spanText = await page.$eval('span.style_myGenreSet__btnText__LxuAP', (element) => element.textContent); //취향 설정하기 버튼의 텍스트 요소 를 spanText에 저장
...
//취향 적용 중 텍스트 변수 선언
  const spanText = await page.$eval('span.style_myGenreSet__btnText__LxuAP', (element) => element.textContent); //취향 적용 중 버튼의 텍스트 요소 를 spanText에 저장
